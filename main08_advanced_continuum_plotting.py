...
# ============================================================
# Load results from advanced continuum experiment
# ============================================================
...
block_sizes = np.array([2000, 1000, 500, 250, 125, 50, 1])
selected_blocks = [2000, 1000, 500, 1]
...
num_sequences = 6
sequence_labels = [f"Seq-{i+1}" for i in range(num_sequences)]

# image folder
image_folder = "images_advanced_continuum"
...
def plot_one_schedule_type(schedule_type):
    ...
    for block_size in block_sizes:
        seq_means = seq_retention_means_by_schedule[schedule_type][block_size]  # shape: (num_runs, 6)

        for run_idx in range(seq_means.shape[0]):
    ...
    df_seq_retention["Sequence"] = pd.Categorical(
        ...
        categories=sequence_labels, # 1 to 6
        ordered=True
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(image_folder, f"{file_prefix}_plot1_training_acquisition_curves.jpg"),
        dpi=600,
        bbox_inches="tight"
    )
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(image_folder, f"{file_prefix}_plot2_avg_retention_loss_by_block_size.jpg"),
        dpi=600,
        bbox_inches="tight"
    )
    plt.show()
    ...
    plt.legend(title="Trained Sequence", ncol=2)
    plt.tight_layout()
    plt.savefig(
        os.path.join(image_folder, f"{file_prefix}_plot3_sequence_level_retention_by_block_size.jpg"),
        dpi=600,
        bbox_inches="tight"
    )
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(image_folder, f"{file_prefix}_plot4_retention_imbalance_by_block_size.jpg"),
        dpi=600,
        bbox_inches="tight"
    )
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(image_folder, f"{file_prefix}_plot5_generalization_loss_by_block_size.jpg"),
        dpi=600,
        bbox_inches="tight"
    )
    plt.show()
